...
class TimelensApp:
    ...
    ENABLED_ROUTE_HANDLERS_CLS_DEV_ONLY: list[type[RouteHandler]] = [
        DevAPIHandler
    ]
    ...
    def __init__(self) -> None:
        ...
        if EnvLoader.is_development():
            for (
                route_handler_cls
            ) in TimelensApp.ENABLED_ROUTE_HANDLERS_CLS_DEV_ONLY:
                self.app.include_router(route_handler_cls(self).get_router())
            self.app.mount(
                "/assets",
    ...
    async def _attach_request_context(
        ...
        if (
            ...
            and EnvLoader.is_debug_bypass_auth_enabled()
        ):
            return await call_next(request)

        async with self.new_db_session() as db_session:
            try:
                await RequestContext.from_request(
