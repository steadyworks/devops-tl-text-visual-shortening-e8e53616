...
class InvalidFilterFieldError(ValueError):
    def __init__(self, field: str, model: type[SQLModel]) -> None:
        super().__init__(
            f"Invalid field '{field}' for model '{model.__name__}'"
        )
...
class AsyncPostgreSQLDAL(
    ...
    AUTO_UPDATE_FIELDS: dict[str, Callable[[], Any]] = {
        "updated_at": lambda: utcnow()
    }
    ...
    @classmethod
    async def create(
        cls, session: AsyncSession, obj_in: CreateSchemaType
    ) -> ModelType:
    ...
    async def update_many_by_ids(
        ...
        try:
            ...
            for field in all_fields:
                column: Optional[SAColumn[Any]] = (
                    cls.model.__table__.columns.get(field)
                )  # pyright: ignore[reportAttributeAccessIssue, reportUnknownMemberType, reportUnknownVariableType]
    ...
    def _build_filter_conditions(
        ...
        if not filters:
            return []
        return [
            cls._resolve_filter_condition(f, op, v)
            for f, (op, v) in filters.items()
        ]
